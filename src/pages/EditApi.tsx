...
const EditApi = () => {
  ...
  const { id } = useParams()
  const { apiConfigs, updateApiConfig, currentApiId } = useApi()

  const existingApi = apiConfigs.find(api => api.id === id)
  ...
  const [formData, setFormData] = useState({
    ...
    provider: existingApi?.provider || 'custom' as const,
    temperature: existingApi?.temperature ?? 0.7,
    maxTokens: existingApi?.maxTokens ?? 2000
  })
