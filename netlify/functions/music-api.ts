...
const handler: Handler = async (event) => {
  ...
  try {
    const { action, keyword, id, limit } = event.queryStringParameters || {}
      ...
      case 'search-qq':
        ...
        try {
          ...
          return {
            statusCode: 200,
            headers,
            body: JSON.stringify(qqData)
          }
        } catch (error) {
          return {
            statusCode: 500,
            headers,
            body: JSON.stringify({ error: 'QQ音乐API请求失败' })
          }
        }
        break
      ...
      case 'url':
        ...
        if (!id) {
          return {
            statusCode: 400,
            headers,
            body: JSON.stringify({ error: '缺少歌曲ID参数' })
          }
        }
